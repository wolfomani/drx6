...
export function EnhancedMessageBubble({ message, isLast, showThinking = true }: EnhancedMessageBubbleProps) {
  const [copied, setCopied] = useState(false)
  const [isThinking, setIsThinking] = useState(false)
  const [showResponse, setShowResponse] = useState(!message.thinking)
  ...
  // Start thinking animation when component mounts (for AI messages with thinking)
  useState(() => {
    if (message.role === "assistant" && message.thinking && showThinking && isLast) {
      setIsThinking(true)
  ...
  if (message.role === "user") {
    return (
      <div className="flex justify-end animate-fade-in">
        <div className="message-bubble user-message bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-2xl px-4 py-3 max-w-[80%] shadow-lg">
    ...
    <div className="space-y-2">
      ...
      {message.thinking && showThinking && (
        <ThinkingDisplay
          ...
          thinkingText={message.thinking.text}
          thinkingTime={message.thinking.duration}
      ...
      {showResponse && (
        <div className="flex justify-start animate-fade-in">
          <div className="message-bubble ai-message glass-effect rounded-2xl px-4 py-3 max-w-[80%] group shadow-lg">
            {/* Model Header */}
            {message.model && (
              <div className="flex items-center space-x-2 mb-2 pb-2 border-b border-gray-600">
                ...
                {message.isDiscussion && (
                  <span className="text-xs bg-purple-600 px-2 py-1 rounded-full">Discussion</span>
                )}
                {message.thinking && (
                  <div className="flex items-center space-x-1 text-xs text-gray-400">
                    <Brain className="w-3 h-3" />
                    <span>فكر لمدة {message.thinking.duration} ثانية</span>
                  </div>
                )}
              </div>
            )}
